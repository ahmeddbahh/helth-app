    ...
    <div class="specific-instructions my-3">
        <div class="row">
            <div class="col-md-6 offset-md-3">
                ...
                <div>
                    <Textarea v-model="data.lab.orthodontal.specificInstructions.text"
                                @change="$v.data.lab.orthodontal.specificInstructions.text.$touch()"
                                class="px-2"
                                :class="{
                                    'border-error':
                                    $v.data.lab.orthodontal.specificInstructions.text.$invalid && $v.data.lab.orthodontal.specificInstructions.text.$dirty,
                                }"
                    ></Textarea>
                    <div class="errors">
                        <small
                        ...
                        v-if="
                            !$v.data.lab.orthodontal.specificInstructions.text.notContainsHtmlTags && $v.data.lab.orthodontal.specificInstructions.text.$dirty
                        "
                        >
                ...
                <div class="build-file" >
                    <div class="form-group">
                        ...
                        <div v-if="data.lab.orthodontal.specificInstructions.fileSrc.length!=0" class="img-box">
                            <div v-for="(file,index) in data.lab.orthodontal.specificInstructions.fileSrc"  :key="index" class="inner-img-box m-1 p-1">
                        ...
                        <span class="text-danger d-block" v-if="data.lab.orthodontal.specificInstructions.fileSrc.length>10">{{lang[local].maxError}}</span>
                    </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    watch:{
        "data.lab.orthodontal.specificInstructions":{
            handler(){
                let notValid = false ;
                this.data.lab.orthodontal.specificInstructions.fileSrc.forEach(file=>{
    ...
    methods:{
        ...
        resetFiles(){
            this.data.lab.orthodontal.specificInstructions.fileSrc = [] ; 
            
        },
        deleteFile(index){
            this.data.lab.orthodontal.specificInstructions.fileSrc.splice(index, 1);
        },
        setInputImage(e){
            this.maxFilesError=false;
            let files=e.target.files;
            // check allowed max number files
            if(this.data.lab.orthodontal.specificInstructions.fileSrc.length<10||(this.fileSrdata.lab.orthodontal.specificInstructions.fileSrc.length+files.length) <10){
                Object.keys(files).forEach(index => {
                    // check if no dublication in uploaded files 
                    if (this.data.lab.orthodontal.specificInstructions.fileSrc.length!=0){
                        let isExistedFile = this.data.lab.orthodontal.specificInstructions.fileSrc.filter(data=>data.name ==files[index].name); 
                        if (isExistedFile.length == 0 ){
                            this.data.lab.orthodontal.specificInstructions.fileSrc.push(customvalidateFile(files[index]));
                        }
                    }
                    else{
                        this.data.lab.orthodontal.specificInstructions.fileSrc.push(customvalidateFile(files[index]));
